(function () {
    ...
    const searchHome = document.getElementsByClassName('searchHome');
    const searchResults = document.getElementById('search-results');
    const searchChitiet = document.getElementsByClassName('searchChitiet');
    searchInput.addEventListener('input', function () {
        ...
        if (filteredProducts.length > 0) {
            filteredProducts.forEach(product => {
                const lengthImg = product.image.length;
                const resultItem = document.createElement('div');
                resultItem.classList.add('result-item');
                resultItem.setAttribute('data-url', product.url);
                resultItem.innerHTML = searchHome.length != 0 ? 
                `   <img src="page/${product.image}" alt="${product.name}" style="width:50px; height:50px;">
                    <span>${product.name}</span>
                
                ` :  searchChitiet.length != 0 ? 
                `   <img src="${product.image.slice(15,lengthImg)}" alt="${product.name}" style="width:50px; height:50px;">
                    <span>${product.name}</span>
                ` :
